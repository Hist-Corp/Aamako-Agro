...
| `Error: Cannot find module 'next/dist/compiled/next-server/server.runtime.prod.js'` (dashboard build — require stack `/vercel/path0/apps/admin/noop.js`) | `next.config.js` pointed Next's **output file tracing root** at the pnpm workspace root (`Dashboard/`) while the Vercel Root Directory is `Dashboard/apps/admin`. The `@vercel/next` builder re-resolves traced paths against `apps/admin`, so a file the builder staged as `Dashboard/node_modules/next/...` was recorded in the trace without its `Dashboard/` segment and the deploy-time require missed. Fix: keep the default tracing root (the app directory — see the vendoring row below) so staged, traced, and required paths agree. |
| Dashboard **dynamic** routes return `500` while **every static route works** — function log shows `Cannot find module 'next/dist/compiled/source-map'` with require stack `…/.pnpm/next@…/node_modules/next/dist/compiled/next-server/server.runtime.prod.js` → `apps/admin/___next_launcher.cjs`, and later the build itself fails with `The framework produced an invalid deployment package for a Serverless Function … files in symlinked directories` / `Cannot find module 'next/dist/compiled/next-server/server.runtime.prod.js'` from `/vercel/path0/apps/admin/noop.js` | pnpm's **isolated linker** installs this app's dependencies as **junctions/symlinks** into `Dashboard/node_modules/.pnpm/…`, which lives **outside** the Vercel Root Directory (`Dashboard/apps/admin`). Four bridge attempts were tried and every one broke: (1) a trace root outside the Root Directory → internal Vercel error; (2) an `outputFileTracingIncludes` glob reaching outside the Root Directory (`../../node_modules/.pnpm/...`) → internal Vercel error; (3) pointing the include through the pnpm junction (`./node_modules/next/...`) → deploy rejected for symlinked directories; (4) `node-linker=hoisted` + workspace-root trace root → the builder re-resolves against `apps/admin`, strips the `Dashboard/` segment, and the deploy require misses `server.runtime.prod.js`. Root cause in all four: the app is a pnpm **workspace package** while Vercel builds only `apps/admin`. **Fix:** stop being a workspace package — the `@aamako/shared-types` sources are vendored at `apps/admin/src/shared-types` (plain imports via `@/shared-types`), the `workspace:*` dependency and `transpilePackages` entry are removed, and `next.config.js` sets **no** `outputFileTracingRoot`/`outputFileTracingIncludes` (defaults keep the trace inside the Root Directory). Keep the project's **Include source files outside of the Root Directory in the Build Step** toggle enabled (§3) — harmless and required by the rest of the guide. |
