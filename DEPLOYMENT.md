...
| Dashboard **dynamic** routes return `500` while **every static route works** — function log shows `Cannot find module 'next/dist/compiled/source-map'` with require stack `…/.pnpm/next@…/node_modules/next/dist/compiled/next-server/server.runtime.prod.js` → `apps/admin/___next_launcher.cjs` | pnpm's default **isolated linker** installs dependencies as **junctions/symlinks** into `Dashboard/node_modules/.pnpm/…`, which is **outside** the Root Directory (`apps/admin`). Next's file tracer resolves those links but drops the targets it cannot place under `outputFileTracingRoot`, so the packaged function ships **without parts of the `next` package itself** and dies at cold start. Only dynamic routes fail, because static routes are pre-rendered HTML and never execute a function; `next start` reads the real `node_modules` tree instead of the traces, so it cannot reproduce the failure locally. Three workarounds were tried and each broke the build: a trace root outside the Root Directory fails with an internal Vercel error; an `outputFileTracingIncludes` glob reaching outside the Root Directory (`../../node_modules/.pnpm/...`) fails the same way; pointing the include through the pnpm junction (`./node_modules/next/...`) packages symlinked directories, which Vercel rejects with `The framework produced an invalid deployment package for a Serverless Function`. **Fix:** `Dashboard/.npmrc` pins **`node-linker=hoisted`** (flat, real-directory `node_modules` like npm — committed, so it applies on Vercel too), and `apps/admin/next.config.js` sets `outputFileTracingRoot` to the workspace root where the hoisted copy lives (outside the Root Directory but inside the workspace — the project's **Include source files outside of the Root Directory in the Build Step** toggle, required by §3, lets the builder package it) plus an `outputFileTracingIncludes` pin on `../../node_modules/next/dist/compiled/**/*` (real directories, no symlinks). Verify with: `node -e "const j=require('./.next/server/app/(dashboard)/pages/[slug]/page.js.nft.json');console.log(j.files.filter(f=>/compiled[\\/]source-map/.test(f)).length)"` → must be > 0. |
